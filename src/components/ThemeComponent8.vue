<template>
  <v-card>
    <v-toolbar flat color="color26" dark>
    ...
    <v-card-text>
      ...
      <v-item-group multiple>
        ...
        <v-item v-for="n in 8" :key="n" v-slot="{ active, toggle }">
          <v-chip class="color31" active-class="color29--text" :input-value="active" @click="toggle">
            Tag {{ n }}
          </v-chip>
        </v-item>
      </v-item-group>
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn color="color32" depressed> Post </v-btn>
    </v-card-actions>
  </v-card>
</template>
